...
test('empty markdown scores lower than well-structured', () => {
  ...
  const structure = result.categories.find((c) => c.label === 'Structure')!
...
test('penalizes multiple h1s', () => {
  ...
  const result = computeScore({ ...defaults, markdown })
  const structure = result.categories.find((c) => c.label === 'Structure')!
  expect(structure.tips).toContain('Use only one h1 heading per page')
})
...
test('penalizes heading level skips', () => {
  ...
  const result = computeScore({ ...defaults, markdown })
  const structure = result.categories.find((c) => c.label === 'Structure')!
  expect(structure.tips).toContain(
...
test('penalizes excessive blank lines', () => {
  const markdown = `# Title\n\n\n\n\nA\n\n\n\n\nB\n\n\n\n\nC\n\n\n\n\nD`
  const result = computeScore({ ...defaults, markdown })
  const noise = result.categories.find((c) => c.label === 'Noise')!
  expect(noise.tips).toContain('Reduce excessive blank lines in output')
})
...
test('penalizes leftover html tags', () => {
  ...
  const result = computeScore({ ...defaults, markdown })
  const noise = result.categories.find((c) => c.label === 'Noise')!
  expect(noise.tips).toContain('Clean up leftover HTML tags in the markdown')
})
...
test('penalizes missing title', () => {
  ...
  expect(metadata.tips).toContain('Include a clear page title as an h1 heading')
})
...
test('penalizes missing links in long content', () => {
  ...
  const result = computeScore({ ...defaults, markdown })
  const metadata = result.categories.find((c) => c.label === 'Metadata')!
  expect(metadata.tips).toContain('Include relevant links for context')
})
...
test('penalizes images without alt text', () => {
  ...
  const result = computeScore({ ...defaults, markdown })
  const metadata = result.categories.find((c) => c.label === 'Metadata')!
  expect(metadata.tips).toContain('Add alt text to images for better context')
})
...
test('penalizes poor compression ratio', () => {
  ...
  const compression = result.categories.find((c) => c.label === 'Compression')!
  expect(compression.score).toBeLessThan(80)
})
...
test('penalizes very long paragraphs', () => {
  ...
  const markdown = `# Title\n\n${longParagraph}\n\n${longParagraph}\n\n${longParagraph}`
  const result = computeScore({ ...defaults, markdown })
  const readability = result.categories.find((c) => c.label === 'Readability')!
  expect(readability.tips).toContain(
