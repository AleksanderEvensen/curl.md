...
First-party [Claude Code](https://code.claude.com) plugin that adds a `fetch` MCP tool, `/curl-md:fetch` slash skill, and an opt-in `WebFetch` redirect hook.
...
```sh
$ npm install --prefix ~/.claude/plugins/npm-cache @curl.md/claude@latest
$ claude plugin install curl-md@curl-md
```
...
| ------------------- | --------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `webfetch_redirect` | `boolean` | `true`  | Redirect Claude’s built-in `WebFetch` tool and tell Claude to retry with curl.md `fetch`, mapping the original prompt to `objective`. |
...
| Skill            | Description                                      |
| ---------------- | ------------------------------------------------ |
| `/curl-md:fetch` | Explicitly tells Claude to use the `fetch` tool. |
